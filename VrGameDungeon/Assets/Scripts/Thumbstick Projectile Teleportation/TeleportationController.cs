...
public class TeleportationController: MonoBehaviour {
	...
	// Reference thumbstick Input Action when scene starts up
	...
	void Start() {
		// We don't want the rayInteractor to be on unless using the forward press on the thumbstick so we deactivate it here
		...
		//_teleportActivate = inputAction.FindActionMap("XRI LeftHand Locomotion " + targetController.ToString()).FindAction("Teleport Mode Activate");
		_teleportActivate = inputAction.FindActionMap("XRI LeftHand Locomotion").FindAction("Teleport Mode Activate");
		...
		//_teleportCancel = inputAction.FindActionMap("XRI LeftHand Locomotion " + targetController.ToString()).FindAction("Teleport Mode Cancel");
		_teleportCancel = inputAction.FindActionMap("XRI LeftHand Locomotion").FindAction("Teleport Mode Cancel");
		...
		//_thumbstickInputAction = inputAction.FindActionMap("XRI LeftHand Locomotion " + targetController.ToString()).FindAction("Move");
		_thumbstickInputAction = inputAction.FindActionMap("XRI LeftHand Locomotion").FindAction("Move");
	...
	// Use the Update function to check for when a teleportation event has occured
	...
	// If those pass, make a teleportation request to the Teleport Provider
	void Update() {
